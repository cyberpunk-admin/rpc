...
type Option struct {
	MagicNumber 	int			//MagicNumber mark it is a geerpc request
	CodeType		codec.Type
...
func (server *Server) ServerConn(conn io.ReadWriteCloser) {
	...
	if err := json.NewDecoder(conn).Decode(&opt); err != nil {
		log.Println("rpc server: option encode error: ", err)
		return
	}
...
// Accept accepts connection on the listener and server requests
...
// Register publishes in the server the set of methods of
// the receiver value that satisfy following conditions:
// - exported method of exported type
// - two arguments, both of exported type
// - the second arguments is a pointer
...
// ServerHTTP implements a http.Handler than answer RPC request
...
// HandleHTTP is a convenient approach for default server to register HTTP handler
func HandleHTTP() {
